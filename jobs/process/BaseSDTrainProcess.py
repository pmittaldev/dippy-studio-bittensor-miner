...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def run(self):
        ...
        if self.datasets is not None:
            self.data_loader = get_dataloader_from_datasets(self.datasets, self.train_config.batch_size, self.sd, training_seed=self.training_seed)
        if self.datasets_reg is not None:
            self.data_loader_reg = get_dataloader_from_datasets(self.datasets_reg, self.train_config.batch_size,
                                                                self.sd, training_seed=self.training_seed)
...
# {self.job.name}
Model trained with Dippy Studio Bittensor Miner
<Gallery />
