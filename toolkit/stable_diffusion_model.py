...
class StableDiffusion:
    ...
    def encode_prompt(
        ...
        # ensure all prompts are strings to prevent tokenizer errors
        validated_prompts = []
        for p in prompt:
            if p is None:
                validated_prompts.append("")
            elif not isinstance(p, str):
                validated_prompts.append(str(p))
            else:
                validated_prompts.append(p)
        prompt = validated_prompts
        ...
        if self.is_v3:
            ...
            # At this point, prompt is already a validated list of strings
            # No need for conditional logic since prompt validation ensures it's never None
            prompt_embeds, pooled_prompt_embeds = train_tools.encode_prompts_flux(
                self.tokenizer,  # list
                self.text_encoder,  # list
                prompt,  # Already validated as list[str]
                truncate=not long_prompts,
                max_length=512,
                dropout_prob=dropout_prob,
